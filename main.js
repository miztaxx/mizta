...
const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['MIZTY - BETA', 'Safari', '2.0.0'] : methodCodeQR ? ['TheMystic-Bot-MD', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
    auth: {
        creds: state.creds,
...
conn.isInit = false;
conn.well = false;
conn.logger.info(`[ ℹ️ ] Connecting...\n`);
...
if (opcion == '1' || methodCodeQR) {
    console.log(chalk.yellow('[ ℹ️ ] Scan QR code.'));
 }}
  if (connection == 'open') {
...
setInterval(async () => {
  ...
  const bio = `• ᴍɪᴢᴛʏ ʙᴏᴛ: ${uptime} | ꜱɪᴍᴘʟᴇ ɪꜱ ʙᴇᴀᴜᴛʏ 🌙`;
